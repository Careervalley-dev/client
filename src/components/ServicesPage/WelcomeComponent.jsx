...
const WelcomeComponent = () => {
  // ]
 return (
    <div className="font-cat mx-auto p-6 lg:p-20 text-white animate-gradient-x">
      <h1 className="text-3xl font-dosis lg:text-5xl font-bold text-black text-center mb-5">
        WELCOME TO THE CAREER VALLEY 
      </h1>
      <p className='text-md lg:text-xl font-bold mb-7 text-black font-dosis text-center'>YOUR GATEWAY TO A BRIGHT FUTURE!</p>
      <p className="text-lg text-center mb-8 px-8 text-black">
      ...
      <h2 className="text-xl font-dosis text-black lg:text-3xl font-bold text-center mb-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {[
          ...
          <div key={index} className="bg-[white] text-black hover:bg-[#4c8474] hover:text-white p-6 rounded-lg shadow-2xl transition-transform duration-300 hover:scale-105">
            <img src={card.img} alt='imaget' className='w-20'/>
            <h3 className="text-xl font-dosis lg:text-2xl font-bold mb-4">{card.title}</h3>
            <p className="text-lg lg:texl-lg">{card.description}</p>
          </div>
        ))}
      </div>
    </div>
